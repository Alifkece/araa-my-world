...
const PlanetPage = (() => {
  ...
  // Two independent CDNs are tried in order, and each script load is
  // retried before giving up — a single flaky request used to be enough
  // to permanently abort the whole scene (the classic "black area, planet
  // never appears" symptom, especially on mobile networks / in-app
  // browsers). Only ONE file is required now (three.min.js) instead of
  // the previous 7 sequential postprocessing/shader files, which cut the
  // number of things that can fail from 7 down to 1.
  const CDN_BASES = [
  ...
  // Tries every CDN base in order (jsDelivr, then unpkg) for the single
  // required file, so one CDN having an outage or being blocked doesn't
  // take the whole page down with it.
  function loadThreeCore() {
    const attempt = (i) => {
      if (i >= CDN_BASES.length) {
        return Promise.reject(new Error("All Three.js CDN sources failed"));
      }
      return loadScriptOnce(CDN_BASES[i] + "build/three.min.js").catch(() => attempt(i + 1));
    };
    return attempt(0);
  }
  ...
  function ensureThree() {
    ...
    loadingPromise = loadThreeCore().catch((err) => {
      // allow a future retry (e.g. after the person taps back in) instead
      // of permanently caching a failed promise
      loadingPromise = null;
      throw err;
    });
    return loadingPromise;
  ...
  function buildPlanet() {
    ...
    const surfaceMat = new THREE.MeshPhongMaterial({
      ...
      emissive: new THREE.Color(0x0d1c3f),
  ...
  function updateCinematic(now, dt) {
    ...
    rimLight.intensity = 1.4 * (1 + glowBoost * 2.4);
  ...
  function enter() {
    ...
    ensureThree()
      ...
      .catch((err) => {
        console.error("Planet scene failed to load (attempt " + loadAttempts + "):", err);
        loadingPromise = null;
        if (loadAttempts < 3 && pageActive) {
          setTimeout(() => { if (pageActive && !sceneReady) enter(); }, 1500);
          return;
        }
        showLoader(false);
