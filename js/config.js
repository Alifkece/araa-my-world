...
const CONFIG = {
  ...
  /* -----------------------------------------------------------------------
     INTRO MUSIC URL
     The intro track is loaded from this URL instead of a local file.
     Planet music stays local (assets/music/planet.mp3) since that already
     works fine.
     ------------------------------------------------------------------- */
  introMusicUrl: "https://raw.githubusercontent.com/Alifkece/media/main/uploads/1784477367739_intro.mp3",
...
All my love, always.`,
  ...
  /* -----------------------------------------------------------------------
     PLANET PAGE — 3D memory sphere (rendered with Three.js)
     ------------------------------------------------------------------- */
  planet: {
    title: "My World Is You",
    subtitle: "You are the center of my universe.",

    photoCount: 130,          // total floating memories (10 photos reused ~13x each). Lower on older phones for extra headroom.
    sphereRadius: 15,         // average distance of the memory sphere shell from the planet (world units)
    sphereJitter: 0.6,        // 0-1, how much photos vary in depth (creates in-front/behind-planet layering)
    planetRadius: 5.2,        // planet size (world units)

    cameraDistance: 34,       // default camera distance from center
    cameraMinDistance: 16,    // closest you can zoom in
    cameraMaxDistance: 60,    // farthest you can zoom out
    autoRotateSpeed: 0.045,   // idle rotation speed of the memory sphere (radians/sec)

    bloomStrength: 1.15,      // planet + star glow intensity
    bloomRadius: 0.55,
    bloomThreshold: 0.15,

    cinematicDuration: 6200,  // ms — length of the double-tap cinematic camera animation
    starCount: 900,           // background particle stars inside the 3D scene, split across 3 depth layers
  },
